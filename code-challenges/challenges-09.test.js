...
function transformToLis(obj){
  // Solution code here...
  return Object.entries(obj).length != 0? (`<li>${Object.entries(obj).join('</li>;<li>')}</li>`).split(',').join(': ').split(';'):[];
}
...
const addValues = (arr) => {
  // Solution code here...
  let newArray= arr.reduce((total,element)=>{
...
const addPurchases = (arr) => {
  // Solution code here...
  let newArray=arr.reduce((total,element)=>{
    return total + element.purchasePrice;
  },0);
  return newArray;
};
...
const reversedString = (str) => {
  // Solution code here...
  return str.split('').reduce((finalStr,element)=>{
    return element + finalStr;
  },'');
};
