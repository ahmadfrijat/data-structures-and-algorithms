...
const count = (target, input) => {
  // Solution code here...
  let newArray = input.reduce((accumulator,index)=>accumulator.concat(index),[]);
  let count=0;
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((accumulator, arr) => accumulator += arr.reduce((accumulator2, value) => accumulator2 += value), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach(element => {
    newArray.push(element.filter(value => typeof value === 'number' && value % 5 === 0).map(value => Math.pow(2, value)))
  });
  return newArray;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let newArray = data.filter(person => person.gender === 'male' || person.gender === 'female')
  let name = newArray.map(person => {
    return person.name;
  });
